...
const Quiz = () => {
  ...
  useEffect(() => {
    const currentId = quizQuestions[currentQuestionIndex]?._id;
    if (currentId && !(currentId in selectedAnswers)) {
      // Reset selected answer for current question if not answered yet
  ...
  const startQuiz = async () => {
    ...
    if (selectedQuestions.length < parseInt(questionCount)) {
      toast({
        description: t("quiz.limitedQuestions", {
